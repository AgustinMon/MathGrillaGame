...
class MathEngine {
  ...
  static Future<void> loadLevels() async {
    try {
      final easyData = await rootBundle.loadString(
        'assets/levels/easy/levels_easy.json',
      );
      final mediumData = await rootBundle.loadString(
        'assets/levels/medium/levels_medium.json',
      );
      final hardData = await rootBundle.loadString(
        'assets/levels/hard/levels_hard.json',
      );
  ...
  static PuzzleLevel generateLevel(
    ...
    if (difficulty == 'easy') {
      ...
      size = size.clamp(11, 15);
    else if (difficulty == 'hard')
      size = size.clamp(16, 20);
    ...
    for (int attempt = 0; attempt < 50; attempt++) {
      ...
      for (int o = 0; o < targetOps * 2 && successfulOps < targetOps; o++) {
        var numberCells = cells.where((c) => c.type == CellType.number).toList()
          ..shuffle();
  ...
  static PuzzleLevel _generateRobustSimpleLevel(
    int levelId,
    int size,
    String difficulty,
  ) {
  ...
  static void _addOperation(
    ...
    double fixedProb = (difficulty == 'easy')
        ? 0.4
        : (difficulty == 'medium' ? 0.35 : 0.3);

    for (int idx in numberIndices) {
      double prob = fixedProb;
      if (idx == 4)
        prob +=
            0.1; // Ligeramente más probable que el resultado sea fijo (ayuda a la deducción)
  ...
  static void _addIntersectingOperation(
    ...
    for (int pos in positions) {
      ...
      int adjustedStartX = horizontal
          ? intersectX - intersectionIndex
          : intersectX;
      int adjustedStartY = horizontal
          ? intersectY
          : intersectY - intersectionIndex;
      ...
      List<int> otherIndices = numberIndices
          .where((idx) => idx != intersectIdx)
          .toList();
      ...
      var existingIntersect = cells.firstWhere(
        (c) => c.x == intersectX && c.y == intersectY,
      );
      int totalFixedInEq =
          fixedIndices.length + (existingIntersect.isFixed ? 1 : 0);
      ...
      for (int i = 0; i < parts.length; i++) {
        ...
        if (existing.isEmpty) {
          bool forceFixed = fixedIndices.contains(i);
          bool forceFooter =
              numberIndices.contains(i) && !fixedIndices.contains(i);
          _createAndAddCell(
            cells,
            x,
            y,
  ...
  static _OpData? _generateOpWithTarget(
    List<String> ops,
    int t,
    int pos,
    int levelId,
    ...
    for (int retry = 0; retry < 50; retry++) {
      ...
      if (pos == 0) {
        // Target es 'a'
        a = t;
        b = _random.nextInt(maxNum) + minNum;
        if (op == '/') {
          b = (_random.nextInt(9) + 2).clamp(2, a > 0 ? a : 10);
          a = (a ~/ b) * b;
        }
      } else if (pos == 1) {
        // Target es 'b'
        b = t;
        a = _random.nextInt(maxNum) + minNum;
        if (op == '/') {
        ...
        res = t;
        if (op == '+') {
          a = _random.nextInt(res.clamp(minNum, 1000));
          b = res - a;
        } else if (op == '-') {
          b = _random.nextInt(maxNum) + minNum;
          a = res + b;
          ...
          a = factors[_random.nextInt(factors.length)];
          b = res ~/ a;
        } else {
          b = (_random.nextInt(10) + minNum);
          a = res * b;
        }
      }
